...
late  double width;
 ...
 late CancelableOperation aiOperation;
  List<int> validMoves = [];
  ChessPiece? selectedPiece = ChessBoard().player1Pieces.first;
  int? checkHintTile = 0;
  Move? latestMove = Move(0,1);
  ...
  void _drawCheckHint(Canvas canvas) {
    if (checkHintTile != null) {
      canvas.drawRect(
        Rect.fromLTWH(
          getXFromTile(checkHintTile!.toInt(), tileSize, appModel),
          getYFromTile(checkHintTile!.toInt(), tileSize, appModel),
          tileSize,
          tileSize,
        ),
